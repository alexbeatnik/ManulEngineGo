...
> runtimes and the VS Code extension reader. Only impl paths differ (`pkg/report`
> here vs `manul_engine/reporting.py` in Engine); HTML markup is generated by each
...
```json
{
  ...
  "persistenceLayer": {
    "runHistory": {
      ...
      "appendedBy": [
        "cmd/manul — sequential and parallel execution paths",
        "cmd/manul — failure/retry paths",
        "pkg/daemon — _run_scheduled_job()"
      ]
    },
    ...
    "htmlReport": {
      ...
      "generator": "pkg/report :: generate_report(summary, output_path)",
      "features": [
